...
|----------------|---------|
| 2.6.36         | Base functionality lands upstream mediation of: <ul><li>File<ul><li>owner conditional</li><li>read, write, link, lock, mmap exec</li></ul></li><li>Execute<ul><li>px, cx, ix, ux, pix, named transitions</li><li>attachment conditional separate from profile name</li></ul></li><li>Change hat<ul><li>single hat</li></ul></li><li>Change Profile</li><li>Capability</li><policy namespaces created through policy load</li></ul> |
| 2.6.37         |         |
| 2.6.38         |         |
| 2.6.39         |         |
| 3.0            |         |
...
| 4.11           |         |
| 4.12           |         |
| 4.13           | <ul><li> apparmorfs policy virtualization<ul><li>the policy entry is now a jumplink to a virtualized policy directory</li><policy directory is now virtualized based on opening task confinement so tasks can only see the subset of policy in their view</li></ul></li><li>speedup path lookups with preallocated buffers</li><li>add namespace level rawdata files<ul><li>profile based rawdata references are now a symlink to the appropriate rawdata file. This allows policy introspection and checkpoint restore to get a single instance of the rawdata without having to read the per profile files and deduping based on hashes</li></ul></li><li>mkdir/rmdir fs based interface for creating namespaces</li><li>revision file interface<ul><li>read current policy revision</li><li>select/poll for when policy changes</li></ul></li><li>add label data query availability to feature set</li><li>support multiple queries per query transaction</li><li>support querying if a profile supports a given mediation type</li><li>revalidate files at exec transition time</li>fine grained ptrace mediation</li><li>domain bounding through profile stacking<ul><li>profile stacking api</li><li>extended change_profile to support profile stacking</li><li>support profile stacks in exec transitions</li></ul></li>      |
